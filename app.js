...
function loadMyTerritories() {
    return db.collection('data').doc('my_territories').get().then(function(doc) {
        if (doc.exists && doc.data().names) {
            myTerritories = doc.data().names;
        } else {
            myTerritories = [];
        }
    }).catch(function(err) {
        console.error('Error cargando territorios propios:', err);
        myTerritories = [];
    });
}

function saveMyTerritories() {
    return db.collection('data').doc('my_territories').set({ names: myTerritories }).catch(function(err) {
        console.error('Error guardando territorios propios:', err);
    });
}
