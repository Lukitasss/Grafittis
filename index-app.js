document.addEventListener('DOMContentLoaded', function() {
    ...
    Promise.all([loadTerritories(), loadMyTerritories()]).then(function() {
        updateStats();
        renderTerritories();
        checkExpirations();
        renderMyTerritories();
    ...
    document.getElementById('editMyTerritoriesBtn').addEventListener('click', toggleWidgetEdit);
    document.getElementById('widgetAddBtn').addEventListener('click', addMyTerritory);

    setInterval(function(){loadTerritories().then(function(){updateStats();renderTerritories();checkExpirations();});},60000);
});
...
function openAddModal() {
    ...
    regionOrder.forEach(function(regionName) {
        ...
        regions[regionName].sort().forEach(function(name) {
...
function importData() {
    ...
    i.onchange=function(e){var r=new FileReader();r.onload=function(ev){try{var d=JSON.parse(ev.target.result);if(Array.isArray(d)){territories=d;saveTerritoriesToFirestore();updateStats();renderTerritories();showNotification('Datos importados','success');}}catch(err){showNotification('Error','danger');}};r.readAsText(e.target.files[0]);};
    i.click();
}
...
function renderMyTerritories() {
    var list = document.getElementById('myTerritoriesList');
    if (!list) return;
    list.innerHTML = '';
    if (myTerritories.length === 0) {
        list.innerHTML = '<div class="widget-empty">Sin territorios</div>';
        return;
    }
    var sorted = myTerritories.slice().sort();
    sorted.forEach(function(name) {
        var item = document.createElement('div');
        item.className = 'widget-item';
        var hasGraffiti = getTerritoryByName(name) !== null;
        item.innerHTML = '<span class="widget-dot ' + (hasGraffiti ? 'dot-active' : 'dot-warning') + '"></span>' +
            '<span class="widget-name">' + name + '</span>' +
            '<button class="widget-remove-btn" data-name="' + name.replace(/"/g, '&quot;') + '">&times;</button>';
        list.appendChild(item);
    });
    list.querySelectorAll('.widget-remove-btn').forEach(function(btn) {
        btn.addEventListener('click', function() {
            var name = btn.getAttribute('data-name');
            myTerritories = myTerritories.filter(function(n) { return n !== name; });
            saveMyTerritories();
            renderMyTerritories();
            showNotification('"' + name + '" removido de Mis Territorios', 'warning');
        });
    });
}

function toggleWidgetEdit() {
    var form = document.getElementById('widgetAddForm');
    var btn = document.getElementById('editMyTerritoriesBtn');
...
function populateWidgetSelect() {
    var sel = document.getElementById('widgetTerritorySelect');
    sel.innerHTML = '<option value="">-- Agregar territorio --</option>';
    var allNames = [];
    ...
    allNames.forEach(function(name) {
        if (myTerritories.indexOf(name) < 0) {
            var opt = document.createElement('option');
            opt.value = name;
...
function addMyTerritory() {
    ...
    if (!name) return;
    if (myTerritories.indexOf(name) < 0) {
        myTerritories.push(name);
        saveMyTerritories();
        renderMyTerritories();
        showNotification('"' + name + '" agregado a Mis Territorios', 'success');
    }
    populateWidgetSelect();
}
