...
function renderTerritories(filter) {
    ...
    if (f.length===0) { document.getElementById('emptyState').style.display='block'; grid.style.display='none'; }
    else { document.getElementById('emptyState').style.display='none'; grid.style.display='grid'; f.forEach(function(t){grid.appendChild(createTerritoryCard(t));}); }
}
...
function createTerritoryCard(territory) {
    ...
    territory.graffitis.forEach(function(g, idx) {
        ...
        if (tr <= 0) {
            ...
            if (horas <= 48) {
                status = 'expired';
                timeText = 'Expirado - ' + (48-horas) + 'h para renovar';
            } else {
                status = 'deleted';
                timeText = 'Eliminado';
            }
            ...
            status = 'warning';
            timeText = Math.ceil(tr/36e5) + 'h restantes';
        } else {
            status = 'active';
            timeText = Math.ceil(tr/864e5) + 'd restantes';
        }
        ...
        html += '<div class="graffiti-date">' + formatDate(new Date(g.date)) + '</div>';
        html += '<div class="graffiti-qty">x' + (g.quantity||1) + '</div>';
